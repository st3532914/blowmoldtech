...
const FEATURED_DEVICES = [
  {
    ...
    sellerType: 'platform', // platform: 平台自营, user: 个人卖家
...
// 快捷入口数据
const QUICK_ACCESS = [
  { id: '1', name: '限时特惠', icon: 'fa-bolt', color: 'text-red-500', bgColor: 'bg-red-100 dark:bg-red-900/30' },
  { id: '2', name: '新品上架', icon: 'fa-tag', color: 'text-blue-500', bgColor: 'bg-blue-100 dark:bg-blue-900/30' },
  { id: '3', name: '品牌专区', icon: 'fa-crown', color: 'text-yellow-500', bgColor: 'bg-yellow-100 dark:bg-yellow-900/30' },
  { id: '4', name: '租赁服务', icon: 'fa-handshake', color: 'text-green-500', bgColor: 'bg-green-100 dark:bg-green-900/30' },
  { id: '5', name: '技术支持', icon: 'fa-headset', color: 'text-purple-500', bgColor: 'bg-purple-100 dark:bg-purple-900/30' },
  { id: '6', name: '金融服务', icon: 'fa-credit-card', color: 'text-indigo-500', bgColor: 'bg-indigo-100 dark:bg-indigo-900/30' },
];
    ...
    <div className={`min-h-screen w-full transition-colors duration-300 ${
            ...
            <div className="relative w-full max-w-3xl">
              ...
              {showSearchHistory && searchHistory.length > 0 && (
                <motion.div 
                  ...
                  className={`absolute top-full left-0 right-0 mt-1 rounded-lg shadow-lg z-20 ${
                    theme === 'dark' ? 'bg-gray-800' : 'bg-white'
                  }`}
                >
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-3 md:grid-cols-6 gap-4">
            {QUICK_ACCESS.map((item) => (
              ...
              >
                <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-2 ${item.bgColor}`}>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <motion.section 
          ...
          className="mb-12"
        >
          <h2 className="text-2xl font-bold mb-8 text-center">为什么选择BlowMoldTech?</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {PLATFORM_ADVANTAGES.map((advantage, index) => (
              <motion.div
                key={index}
                whileHover={{ y: -5 }}
                className={`p-6 rounded-16 transition-all ${
                  theme === 'dark' 
        ...
        <section className="mb-12">
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center">
              ...
              <div className="flex items-center px-2 py-0.5 bg-red-100 text-red-600 rounded text-sm">
            ...
            >
              更多优惠 <i className="fas fa-chevron-right text-sm"></i>
            </button>
          </div>
              ...
              >
                <div className="relative">
                  ...
                  <div className="absolute top-0 left-0 w-16 h-16">
                    <div className="bg-red-500 text-white text-xs font-bold transform -rotate-45 translate-x-[-25%] translate-y-[-25%] py-1 w-[140%] text-center">
        ...
        <section className="mb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {FEATURED_DEVICES.filter(device => device.sellerType === 'platform').map((device) => (
              <DeviceCard 
                key={device.id}
                device={device}
        ...
        <section className="mb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {FEATURED_DEVICES.filter(device => device.sellerType === 'user').map((device) => (
              <DeviceCard 
                key={device.id}
                device={device}
                onClick={() => handleDeviceClick(device.id)}
        ...
        <section className="mb-12">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {['西门子', 'ABB', '海天', '震雄', '伊之密', '克劳斯玛菲'].map((brand, index) => (
              <motion.div
                key={index}
                whileHover={{ scale: 1.05 }}
        ...
        <motion.section 
          ...
          className={`p-8 rounded-16 mb-12 ${
            theme === 'dark' ? 'bg-gray-800' : 'bg-gradient-to-r from-blue-50 to-indigo-50'
          }`}
        >
          <div className="max-w-3xl mx-auto text-center">
            <h2 className="text-2xl font-bold mb-4">加入BlowMoldTech会员</h2>
            <p className={`mb-6 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
              成为会员享受专属折扣、优先看货、技术支持等多重特权，让您的设备采购更加便捷。
            </p>
